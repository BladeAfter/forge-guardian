...
grant all on public.referrals,public.referral_purchase_events,public.economy_settings,public.referral_commissions,public.player_notifications,public.referral_bonus_rules,public.referral_bonus_claims to service_role;
...
declare v_current uuid:=p_buyer_id;beneficiary uuid;lvl integer;rate numeric;paid numeric;total_paid numeric:=0;is_excluded boolean;
begin
 ...
 values(p_buyer_id,p_purchase_id,lower(p_event_type),round(p_amount_fc,3),upper(p_source_currency),p_source_amount,p_eligible and not is_excluded)
 on conflict(purchase_id) do nothing;
 if not found then return jsonb_build_object('duplicate',true,'totalPaid',0);end if;
 if not p_eligible or is_excluded then return jsonb_build_object('duplicate',false,'totalPaid',0);end if;
 for lvl in 1..3 loop
   select inviter_id into beneficiary from referrals where user_id=v_current;
